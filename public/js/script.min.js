document.addEventListener('DOMContentLoaded', () => {
    ...
    btnAddLocation.addEventListener('click', () => {
        ...
        if (selectInstitucion.value.startsWith('new_') && !txtNewInstitucion.value.trim()) return Swal.fire('Falta Institución', 'Ingresa el nombre de la nueva institución', 'warning');
    ...
    function renderLocationsTable() {
        ...
        if (registeredLocations.length === 0 && locationsList.length === 0) {
            locationsTableBody.innerHTML = '<tr id="emptyRow"><td colspan="6" style="text-align:center; padding: 15px; color: #999;">No has agregado sedes aún.</td></tr>';
            return;
        }
        ...
        registeredLocations.forEach((loc, index) => {
            ...
            tr.innerHTML = `
                ...
                <td style="padding: 5px; text-align: center;">
                    <button type="button" class="btn-primary btn-blue" onclick="openUpdateModal(${index})" style="padding: 2px 8px; font-size: 0.8rem; margin-right: 6px;">📝</button>
                    <button type="button" class="btn-cancel" onclick="removeRegisteredLocation(${index})" style="padding: 2px 8px; font-size: 0.8rem;">🗑️</button>
                </td>
            `;
    ...
    window.openUpdateModal = (index) => {
        ...
        Swal.fire({
            ...
            confirmButtonText: 'Agregar actualización',
    ...
    function resetForm() {
        ...
        selectActividad.innerHTML = '<option value="">-- SELECCIONE --</option><option value="new_activity">➕ NUEVA ACTIVIDAD...</option>';
        ...
        fetch(`/api/project/${bpin}`)
            ...
            .then(d => {
                ...
                if (d.activities.length === 1) {
                    const singleActivity = d.activities[0];
    ...
    document.getElementById('btnCleanDB').addEventListener('click', async (e) => {
        ...
        const confirm = await Swal.fire({
            title: '¿Estás seguro?',
            text: 'Esto eliminará TODOS los datos.',
            icon: 'warning',
            showCancelButton: true,
    ...
    document.getElementById('btnExport').onclick = async () => {
        const result = await Swal.fire({
            ...
            text: '¿Qué deseas descargar?',
            icon: 'question',
            showCancelButton: true,
            showDenyButton: true,
            confirmButtonText: 'Plantilla carga masiva',
            denyButtonText: 'Información almacenada',
            cancelButtonText: 'Cancelar'
        });
